...
.ub{font-size:.6rem;padding:2px 8px;border-radius:4px;text-transform:uppercase;letter-spacing:1px}.ub.admin{background:rgba(255,215,0,.15);color:var(--gold)}.ub.user{background:rgba(136,136,160,.15);color:var(--text2)}
...
async function doLogin(){const u=document.getElementById('login-user').value.trim(),p=document.getElementById('login-pass').value;if(!u||!p){showToast('Felder ausfüllen',1);return}
  const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});const d=await r.json();if(!r.ok){showToast(d.error||'Fehler',1);return}currentUser=d;showScreen('dashboard');updateNav()}
async function doLogout(){await fetch('/api/logout',{method:'POST'});currentUser=null;showScreen('home');updateNav()}
async function changePw(){const o=document.getElementById('pw-old').value,n=document.getElementById('pw-new').value,n2=document.getElementById('pw-new2').value;if(!o||!n){showToast('Felder ausfüllen',1);return}if(n!==n2){showToast('Nicht überein',1);return}
  const r=await fetch('/api/change-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({OldPassword:o,NewPassword:n})});const d=await r.json();if(d.error){showToast(d.error,1);return}showToast('Geändert',0);showScreen('dashboard')}
...
async function saveAIConfig(){const b={Provider:document.getElementById('ai-provider').value,Model:document.getElementById('ai-model').value,IntroDelay:document.getElementById('ai-intro-delay').value};
  ...
  await fetch('/api/ai-config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)});showToast('Gespeichert',0);await loadSounds();loadAdmin()}
async function testAI(){const st=document.getElementById('ai-status');st.textContent='Teste...';const p=document.getElementById('ai-provider').value;const k=p==='anthropic'?document.getElementById('ai-anthropic-key').value.trim():document.getElementById('ai-openai-key').value.trim();const m=document.getElementById('ai-model').value;const r=await fetch('/api/test-ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({Provider:p,Key:k,Model:m})});const d=await r.json();st.textContent=d.message;st.style.color=d.ok?'var(--correct)':'var(--wrong)'}
function renderUsers(users){document.getElementById('users-list').innerHTML=users.map(u=>{const dis=u.isOnlyAdmin;return '<div class="user-row"><div class="user-info"><span>'+esc(u.username)+'</span><span class="ub '+(u.isAdmin?'admin':'user')+'">'+(u.isAdmin?'Admin':'User')+'</span></div><div class="user-actions">'+(u.isAdmin?'<button class="ib"'+(dis?' disabled':' onclick="toggleAdmin('+u.id+',false)"')+'>Degradieren</button>':'<button class="ib" onclick="toggleAdmin('+u.id+',true)">Zum Admin</button>')+'<button class="ib danger"'+(dis?' disabled':' onclick="deleteUser('+u.id+',\''+esc(u.username)+'\')"')+'>Löschen</button></div></div>'}).join('')}
...
async function deleteUser(id,name){if(!confirm(name+' löschen?'))return;await fetch('/api/users',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});loadAdmin()}
async function toggleAdmin(id,make){await fetch('/api/users',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,isAdmin:make})});loadAdmin()}
...
function uploadSound(type,inputId){const input=document.getElementById(inputId);const f=input&&input.files[0];if(!f){showToast('Datei auswählen',1);return}if(f.size>20*1024*1024){showToast('Max 20 MB',1);return}
  const fd=new FormData();fd.append('type',type);fd.append('file',f);const xhr=new XMLHttpRequest();xhr.onload=async function(){try{const d=JSON.parse(xhr.responseText);if(d.ok){showToast('Hochgeladen!',0);input.value='';await loadSounds();loadAdmin()}else showToast(d.error||'Fehler',1)}catch(e){showToast('Serverfehler',1)}};xhr.onerror=()=>showToast('Netzwerkfehler',1);xhr.open('POST','/api/sounds');xhr.send(fd)}
async function deleteSound(type){await fetch('/api/sounds',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({type})});showToast('Entfernt',0);await loadSounds();loadAdmin()}
...
async function clearLogs(){if(!confirm('Alle Logs unwiderruflich löschen?'))return;await fetch('/api/logs',{method:'DELETE'});showToast('Geleert',0);loadLogs()}
